-- TASK 11 SUBMISSION
-- ============================================================
-- Database Systems - Scalar SQL Functions
...
-- Task A1
SELECT CustID, CustName AS Original, TRIM(CustName) AS CleanedName FROM Customer;

-- Task A2
SELECT CustID, UPPER(CustName) AS UpperName, LOWER(CustName) AS LowerName FROM Customer;

-- Task A3
SELECT CustID, TRIM(CustName) AS TrimmedName, CHAR_LENGTH(TRIM(CustName)) AS NameLength FROM Customer;

-- Task A4
SELECT CONCAT('Dear ', TRIM(CustName), ', welcome!') AS Greeting FROM Customer;

-- Task A5
SELECT CustName, SUBSTRING(Email, 1, LOCATE('@', Email)-1) AS Username
FROM Customer WHERE Email IS NOT NULL;

-- Task A6
SELECT CustName, SUBSTRING(Email, LOCATE('@', Email)+1) AS Domain
FROM Customer WHERE Email IS NOT NULL;

-- Task A7
SELECT CustID, CustName, LEFT(TRIM(CustName), 3) AS FirstThreeCharacters FROM Customer;

-- Task A8
SELECT CustName, CONCAT(LEFT(Phone, 4), 'XXX-XXXX') AS MaskedPhone
FROM Customer WHERE Phone IS NOT NULL;

-- Task A9
SELECT ProdID, REPLACE(ProdName, ' ', '-') AS SlugName FROM Product;

-- Task A10
SELECT ProdID, LPAD(ProdID, 5, '0') AS PaddedID FROM Product;
...
-- Task A12
SELECT CustID, TRIM(CustName) AS CustName,
